...
if __name__ == '__main__':
    while True:
        nome_arquivo = input("\nDigite o nome do arquivo (.txt) que deseja cifrar: ")
        ...
        if conteudo == "":
            print(f"O arquivo {nome_arquivo} está vazio. Por favor selecione outro arquivo.")
            continue
        ...
        try:
            userchoice = int(input('Digite a opção desejada (1, 2, 3 ou 0): '))

        except ValueError:
            print('Opção inválida, tente novamente.')
            continue
        if userchoice == 0:
            print("Encerrando o programa...Fim!")
            break
